...
public:
	...
	typedef std::function<bool(combination_nd<Dim>)> allowed_predicate;
...
protected:
	...
	const allowed_predicate allowed;
...
public:
	...
	static std::shared_ptr<NnDomain<Dim>> make_binary_combination(
		typename BinaryString<Dim>::allowed_predicatet allowed
	) { // allowed_predicate
		return std::make_shared<RectangleDomain<Dim>>(allowed);
	}
