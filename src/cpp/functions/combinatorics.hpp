...
namespace TestFunctions {
	...
	std::function<double(const combination_nd<Dim>&)> make_knapsack(double weights, const std::vector<value_weight_pair>& v_w_pairs) {
		if (Dim != v_w_pairs.size())
			throw std::exception("Mismatch between knapsack problem dimension and size of given pairs");
